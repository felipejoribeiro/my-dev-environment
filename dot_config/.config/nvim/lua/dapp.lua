...
PANE = {}
local attachToReactNative = function() print("dap not found") end
local startReactNative = function() print("dap not found") end
local compileAndroidApp = function() print("dap not found") end
...
if pcall(require, "dap") then
  ...
  local function set_keymaps_for_react_native()
    local isReactNativeProject = function()
      ...
      return string.find(path, "mobile") ~= nil
    end

    if isReactNativeProject() then
      vim.api.nvim_buf_set_keymap(0, 'n', '<space>da', '<cmd>lua require("dapp").attachToReactNative()<CR>', { noremap=true, silent=true })
      vim.api.nvim_buf_set_keymap(0, 'n', '<space>js', '<cmd>lua require("dapp").startReactNative()<CR>', { noremap=true, silent=true })
      vim.api.nvim_buf_set_keymap(0, 'n', '<space>jc', '<cmd>lua require("dapp").compileAndroidApp()<CR>', { noremap=true, silent=true })
    end
  end
